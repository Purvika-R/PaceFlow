...
const initials = (person) =>
  person?.name?.split(' ').map((part) => part[0]).join('').slice(0, 2).toUpperCase() || 'PR';

const formatDate = (date) =>
  date ? new Intl.DateTimeFormat('en', { month: 'short', day: 'numeric' }).format(new Date(date)) : 'No due date';
...
export function useWorkspaceData() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [users, rawProjects, rawTasks] = await Promise.all([
      ...
      const tasks = rawTasks.map((task) => ({
        ...task,
        id: task._id,
        project: task.project?.name || 'Unassigned project',
        assignee: task.assignee?.name || 'Unassigned',
        status: displayStatus[task.status] || task.status,
        priority: titleCase(task.priority),
        dueDate: formatDate(task.dueDate),
        rawProjectId: task.project?._id || task.project,
        rawAssigneeId: task.assignee?._id || null,
        rawDueDate: formatDateInput(task.dueDate),
      }));
      ...
      const projects = rawProjects.map((project) => {
        ...
        return {
          ...
          progress: projectTasks.length
            ...
            : 0,
          priority: projectTasks.some((task) => task.priority === 'High')
            ? 'High'
            : projectTasks.some((task) => task.priority === 'Medium')
            ? 'Medium'
            : 'Low',
          dueDate: dueDates[0] || 'No due date',
          members: [initials(project.owner)],
          rawStatus: project.status,
          rawOwnerId: project.owner?._id || project.owner,
        };
      });
